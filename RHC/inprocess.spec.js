describe('Dataingest creation ', () => {
    ...
    it('Verify Eligibility by bb user', function() {
    var i = 0;
    for (var i = 0; i < 10 ; i++) {
  ...
  cy.wait(5000)
  cy.contains(' Verify Eligibility RHC ', { timeout: 2000}).click({ multiple: true });
  ...
  cy.get('mat-radio-button.mat-radio-button.mat-accent', { timeout: 2000}).find('.mat-radio-label-content').contains('Yes').click({ multiple: true });
  ...
  cy.wait(20000);
  cy.get('mat-list-option.mat-list-item.mat-list-option.mat-focus-indicator.mat-accent.ng-star-inserted', { timeout: 5000}).find('.mat-list-item-content > .mat-list-text > [fxlayout="row"] > .taskalias', { timeout: 2000}).contains(' Complete Full Registration ', { timeout: 2000}).click({ multiple: true })
  cy.get(5000);
  cy.get('[type=checkbox]:eq(0)', { timeout: 5000}).check({force: true});
